...
programs as the \G{normative} key drivers of the science requirements for the
project. Their selection was the result of discussions within the
consortium and reflects the input of
\begin{itemize}
\item the three National Research Council studies\footnote{
...
http://docushare.lsstcorp.org/docushare/dsweb/Get/Document-220},
...
goals (separate documents will deal with the data management
requirements\footnote{\G{The two high-level documents derived from
this document and other constraints are the LSST System Requirements
Document and the LSST System Architecture Document.}}).
...
\B{SWG report} but the long-lived
...
\subsection{Constraining Dark Energy and Dark Matter}
...
statistical errors by a factor of $\sim$3. \B{While the statistical error is
...
or are uncorrelated between observations due to seeing.}
\G{
...
days and with exposures of 30 seconds, LSST will discover \B{250,000}
...
of finding \B{90\%} \G{84\%} of the PHAs with diameters larger than 140 m within
ten years.
...
from exposure time \G{limits}, \B{and the requirement for at least two filters
...
\B{Gamma-Ray Large Area Space Telescope (GLAST)}
...
\B{the SDSS has demonstrated such
studies out to distances of 15 kpc using data with $r<21.5$).}

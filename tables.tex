...
\B{The daytime filter change should be accomplished in less than
...
camera. \B{, and the maximum time, in hours, to change which filters are housed
in the camera.}}
...
measurements of fast transients. \B{, and will prevent a significant loss of
...
\hline
\end{tabular}
\caption{Single image depth in the $r$ band (SNR=5 for point sourcesss).
The D1 and Z1 values are expressed on the AB magnitude scale
and assume \R{a source with spectral energy distribution $F_\nu$=constant,}
\B{an A0V star} fiducial seeing of 0.7 arcsec (FWHM), fiducial dark sky brightness of
21 mag/arcsec$^2$, airmass of 1.0, and a total exposure time of 30 sec.
\B{(the computed depths depend on lunar phase and mirror coatings; aged, unprotected
aluminum is assumed for the latter and a 3 day old moon for the former).}
...
\B{with the corresponding {\it implied} overall system throughput of
...
\G{NEW DB1 (mag)}  &    23.4   &    24.6  &     24.3   &  23.6 &  22.9 & 21.7 \\
%\hline
...
the $r$-band sky brightness \B{at solar maximum} of 21 mag/arcsec$^2$
(both seeing and sky brightness are converted to the appropriate band using standard
expressions, \B{for a 3 day old moon} as implemented in the LSST Exposure Time
Calculator). \B{The {\it implied} overall system throughput as a function of
...
\B{The design specification values for
...
0.1-0.2 mag.}
The \R{assumed} bandpasses \B{used in these computations} are illustrated in
Appendix C. \B{A preliminary version of the LSST Exposure Time Calculator
...
drop below values needed to meet minumum specifications for image depths
listed in Table~\ref{ImageDBtable}.}

\B{A decrease in throughput, DT1, by 30\% of the listed
...
\B{The design specification value allows a saturation level $\sim$1 mag brighter than for
...
system. \B{It is also required that the pixel size be smaller than one third
of the delivered image size (FWHM) in median atmospheric seeing
($<$0.22 arcsec for the design specification from Table ~\ref{seeingT}).}
...
\B{Assuming that the atmospheric seeing
increases with airmass, $X$, as $\propto X^{0.6}$, the design
specification for the allowed error budget due to the system is
...
\B{{\bf Table~\ref{Tellip2} will be moved to the LSST System Requirements Document.}
The supporting text was moved to the Full Survey Specifications (see Table~\ref{fullEllip}).}

\B{{\bf Specification:} The median LSST image (two per visit) must have the median
...
\B{We do not specify values on larger scales because by supposition each
...
A broad-band photometric system, such as LSST, aims to \B{measure the}
...
\begin{equation}
\label{SDef}
         S_b(\lambda) = S_b^{sys}(\lambda) \times S^{atm}(\lambda).
\end{equation}
...
specified below ($<$1\%), both $S_b^{sys}(\lambda)$ and $S^{atm}(\lambda)$
...
$S^{atm}(\lambda)$ are {\it not} required (c.f. eq.~\ref{PhiDef}) to
...
to {\it both} the bright and faint ends and constrain the nonlinearity of the
...
\B{{\bf Table~\ref{ghosting} will be moved to the LSST System Requirements Document.}}


\B{{\bf Specification:} The measured photometric error using repeated observations
...
\B{These requirements limit the impact of ghosting. The first requirement effectively
...
\B{Other issues, such as maximum sensor area loss and up/down time, will be
...
towards the specificiations listed in this Section. For example, if the photometric
...
Detailed simulations of LSST operations \B{available at the time of writing
\B{(the so-called {\it cronos.92} Operations Simulation run)}}
...
distribution at a redshift of $\sim$2. \B{The expected number of galaxies
...
(\B{about 200 million F/G} main sequence stars will enable kinematic and
...
\B{(version {\it cronos.92})} as well as allocation of about 10\% of the
...
\B{The total depth is computed using a
...
\G{The coadded image depth losses due to airmass greater than unity are not
...
\B{Table~\ref{TareaPM} will be replaced by Table~\ref{TablePARAPM}}

\B{{\bf Specification:} At least
...
\hline
\end{tabular}
\caption{\B{The minimum area with revisit time distribution allowing accurate
parallax and proper motion measurements.}}
\label{TareaPM}
\end{table}

\B{
The requirements are derived using a requirement of proper motion accuracy of 0.2 mas/yr
...
covariance (again, for the best-fit elliptical Gaussian).  \B{We then fit
...
so-called ``B mode" shear (non-zero $E_X$). Using as priors the measured FPA nonflatness, the wavefront
...
\B{To be moved to the LSST System Requirements Document:
Tests based on data obtained by Subaru telescope suggest that these values are
achievable: the atmospheric contribution to residual ellipticity correlation averages
down with the number of exposures. This assumes that effective use of the priors
discussed in the single-image ellipticity specifications is made, otherwise recurring
systematics will dominate the averaged-down random atmosphere effects. This requires
optimal correction of the PSF in each image, and fits a model for each galaxy in each
image [MutiFit]. An alternative approach being explored is to utilize an image of the
coadded PSF, and fit a forward model of the (known) sum PSF convolved with a galaxy
model to the stacked image [StackFit].}
...
\G{Detailed requirements on data processing and management will be described
...
\item {\bf Level 3} data products will be created by science teams external to the
project using suitable Applications Programming Interfaces (APIs) that will be provided

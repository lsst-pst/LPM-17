...
independent of the existing baseline design (see Appendix \ref{app:baseline}), and of
...
the bandpasses is illustrated in Appendix \ref{app:bandpasses}.
...
\ref{app:bandpasses}) is limited to a 0.05 mag bias in the $u$ band measurement ($<$0.01 mag
...
have motivated the baseline design parameters listed in Appendix \ref{app:baseline}.
The assumed bandpasses are illustrated in Appendix \ref{app:bandpasses}. Note that there is no
...
Cerro Pach\'{o}n site is illustrated in Appendix \ref{app:seeing}.
...
\ref{ABmag}, $m^{std}_{cat}$ is the cataloged LSST magnitude, \newtext{both evaluated using
...
normalized system response ($\phi$). \newtext{This specification applies to the cataloged LSST
magnitudes, $m^{std}_{cat}$ (see eq.~\ref{eq:mstdcat}), for appropriately chosen main sequence
stars (e.g., non-variable stars color-selected from the main stellar locus).} In practice,
...
As a result of these studies, the adopted baseline design (see Appendix \ref{app:baseline})
...
Appendix \ref{app:unicadence} as a minimalistic scenario. It shows that $\sim$10\% of the
...
visits per night  (see Appendix \ref{app:unicadence}).
